...
function createSecurityApi({ vault, log }) {
    ...
    async function handle(req, res, parsedUrl) {
        ...
        if (route === 'admin/setup' && method === 'POST') {
            ...
            return send(200, { token: signToken({ role: 'admin' }, data.serverSecret, ADMIN_TTL_MS), exp: Date.now() + ADMIN_TTL_MS });
        }
        ...
        if (route === 'vault' && method === 'GET') {
            ...
            for (const [key, s] of Object.entries(data.sections || {})) {
                out[key] = { scope: s.scope, name: s.name, pinRelock: s.pinRelock, content: s.content };
            }
