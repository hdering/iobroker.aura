 ...
 *
 * @param {object} input
 * @param {{where: string, widgets: object[], pinLocked?: boolean}[]} input.places tabs, popup views and groups
...
function auditDashboard(input) {
    ...
    for (const place of places) {
        // A PIN-protected tab arrives as a redacted stub with no widgets (see
        // auraConfig.pinLock). Reported from use: it was filed under „ohne Widgets
        // — steht im Menü, zeigt eine leere Seite“, which is what a lost tab looks
        // like. It is not checked and it is not empty; the render says so.
        if (place.pinLocked) {
            protectedPlaces.push(place.where);
            continue;
        }
    ...
    return {
        findings,
        stats: {
            places: places.length - protectedPlaces.length,
            widgets: widgetCount,
            datapoints: dpCount,
            protectedPlaces,
        },
...
function renderAudit(result, scope) {
    ...
    const locked = (stats.protectedPlaces || []).length;
    const head =
        ...
        // Coverage, not a finding: what was skipped has to be said, or the answer
        // claims to have looked at the whole dashboard when it did not.
        (locked
            ? ` ${locked} PIN-geschützte Ansicht(en) NICHT geprüft (Inhalt liegt server-seitig im Tresor, ` +
              `nicht in dieser Konfiguration): ${itemList(stats.protectedPlaces)}.`
            : '');
    if (!findings.length) {
