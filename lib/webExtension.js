...
function rewriteHtml(html, baseUrl) {
    ...
    // Add explicit action to <form> tags that have none (e.g. Django login).
    // Without this the browser posts to window.location which after
    // history.replaceState is the aura host — not the internal server.
    out = out.replace(/<form\b([^>]*)>/gi, (match, attrs) => {
        if (/\baction\s*=/i.test(attrs)) return match;
        return `<form${attrs} action="${toProxy(baseUrl)}">`;
    });
    // Rewrite srcset (comma-separated url [descriptor] list)
    out = out.replace(
        /\bsrcset(\s*=\s*)(["'])([^"']*)\2/gi,
        (_, eq, q, val) => {
    ...
    // Use the actual path+search from the target URL so the SPA router sees
    // the right route (e.g. '/dashboard') instead of '/aura/proxy'. Using '/'
    // was wrong for the login page: after a successful login Django redirected
    // to '/' which resolved to the web-adapter's own root page.
    const targetPathForReplace = JSON.stringify(
        new URL(baseUrl).pathname + new URL(baseUrl).search,
    );
    const wsSnippet  = `<script>(function(){` +
        ...
        // Rewrite the browser URL to the real target path so SPA routers
        // (Angular, Vue, React) see e.g. '/dashboard' and not '/aura/proxy'.
        `history.replaceState(history.state,'',${targetPathForReplace});` +
