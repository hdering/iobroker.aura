...
class Aura extends utils.Adapter {
  ...
  async onReady() {
    ...
    // system.adapter.* triggers an adapter restart, so we must avoid no-op writes.
    {
      ...
      const wantFrontend = base ? `${base}/aura/` : '%protocol%://%ip%:8082/aura/';
      const wantBackend  = base ? `${base}/aura/#/admin` : '%protocol%://%ip%:8082/aura/#/admin';
      try {
        const obj = await this.getForeignObjectAsync(`system.adapter.${this.namespace}`);
        if (obj) {
          const curLinks  = obj.common.localLinks || {};
          const wantLinks = { 'Aura Frontend': wantFrontend, 'Aura Backend': wantBackend };
