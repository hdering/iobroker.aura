...
function rewriteHtml(html, baseUrl) {
    ...
    // Inject WebSocket interceptor so ws:// URLs constructed by app JS are
    // routed through /aura/proxyws?url=ws://... instead of failing directly.
    const tgtNoProto = targetOrigin.replace(/^https?:/, '');  // e.g. //192.168.188.22:8000
    // WS protocol follows the target origin's protocol, not the proxy page's
    const wsProto = targetOrigin.startsWith('https:') ? 'wss:' : 'ws:';
    const wsSnippet = `<script>(function(){` +
        `var _W=window.WebSocket,pp=${JSON.stringify(PROXY_PATH)};` +
        `window.WebSocket=function(u,p){` +
        `try{` +
            `var a=new URL(u.replace(/^wss?:/,'https:'),location.href);` +
            `if(a.origin===location.origin&&a.pathname.indexOf(pp)===0){` +
                `var path=a.pathname.slice(pp.length);` +  // e.g. "ws/status/"
                `u='/aura/proxyws?url='+encodeURIComponent(${JSON.stringify(wsProto)}+${JSON.stringify(tgtNoProto)}+'/'+path.replace(/^\\/+/,'')+(a.search||''));` +
            `}` +
        `}catch(e){}` +
        `return new _W(u,p);` +
        `};` +
