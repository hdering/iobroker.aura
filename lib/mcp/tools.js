...
const INSTRUCTIONS = [
    ...
    'rendered, what the content needs, what scrolls, and which widgets draw nothing at all. aura_measure',
    'works off a table measured once, which ages with every change to the styling; where the two',
    'disagree, the browser is right. A card that is simply bigger than it needs to be is reserve and is',
    'reported as no such disagreement. A tab nobody has opened has no measurement — ask the user to',
    'open it.',
...
const TOOLS = [
    ...
    },
    {
        ...
        description:
            ...
            'line, including the ones that draw nothing at all. Only tabs somebody has had OPEN since the ' +
            'adapter started have a measurement (a hidden tab renders nothing), and every entry says how old ' +
            'it is — ask the user to open the tab if the one you need is missing.',
        inputSchema: {
            type: 'object',
            properties: {
