...
function createSecurityApi({ vault, log }) {
    const noop = () => {};
    ...
    async function handle(req, res, parsedUrl) {
        ...
        // POST /vault/remove {key} — Bearer admin. Forget one view: its PIN was
        // removed in the editor („PIN entfernen“).
        //
        // Called only *after* the save that wrote the view's content back into
        // config.dashboard in plaintext — until then the vault holds the only copy.
        // Idempotent, so a second click (or a retry) is not an error. The adapter
        // rebuilds `sections` from every save that still carries a PIN, so this is
        // the case that path cannot cover: the last protected view losing its PIN,
        // where the config has no PIN left to trigger a rebuild.
        if (route === 'vault/remove' && method === 'POST') {
            if (!adminAuthed(req, data)) return send(401, { error: 'unauthorized' });
            const body = await readJsonBody(req);
            const key = String((body && body.key) || '');
            if (!key) return send(400, { error: 'key required' });
            const removed = !!(data.sections && data.sections[key]);
            if (removed) {
                delete data.sections[key];
                vault.save(data);
                logger.info(`aura: PIN vault — ${key} forgotten (PIN removed in the editor)`);
            }
            return send(200, { key, removed });
        }
