...
const { flushHydrated } = require('./protectedView.js');
...
function renderReportEntry(msg) {
    ...
    return {
        ...
        entry: {
            ...
            // Measured off-screen on a tab nobody had open (RenderProbe) instead
            // of on a screen in use. Kept so the answer can say which it is: the
            // grid and the width are the real ones, but camera and iframe cards
            // are empty boxes there.
            probe: !!m.probe,
...
/**
 * Ask every live frontend to measure one tab off-screen.
 *
 * @param {object} adapter
 * @param {string} tabId
 * @returns {Promise<number>} the timestamp written, to tell a fresh report from an old one
 */
async function requestRenderProbe(adapter, tabId) {
    const ts = Date.now();
    await adapter.setStateAsync('info.renderProbe', { val: JSON.stringify({ tabId, ts }), ack: true });
    return ts;
}
...
/**
 * A PIN-protected section or tab does not arrive here with its content: the
 * adapter moves that into the server-side vault and leaves a stub behind (see
 * lib/security/dashboardVault.js). The stub carries `pinProtected: true` and an
 * EMPTY widget list — and read literally that is indistinguishable from a tab
 * whose widgets were lost.
 *
 * Reported from use: a protected section read as „0 Widget(s), endet auf Zeile 0“,
 * aura_tab answered `widgets: []` and aura_review filed it under „ohne Widgets —
 * steht im Menü, zeigt eine leere Seite“. Half an hour of hunting for a data loss
 * that had not happened. Everything that prints or walks a tab therefore asks
 * `pinLock` first and says PROTECTED where it would otherwise say empty.
 */
function pinLock(section, tab) {
    if (section && section.pinProtected) {
        return { locked: true, scope: 'section', name: section.name };
    }
    if (tab && tab.pinProtected) {
        return { locked: true, scope: 'tab', name: tab.name };
    }
    return { locked: false, scope: null, name: null };
}

/** The one label for a locked view. Never „leer“, never a widget count. */
const PIN_LOCKED_LABEL = 'PIN-geschützt, Inhalt nicht einsehbar';

/** Why an answer says that — printed once per answer, not per tab. */
const PIN_LOCKED_NOTE =
    `${PIN_LOCKED_LABEL}: der Inhalt liegt server-seitig im PIN-Tresor und kommt nur nach dem Entsperren ` +
    'im Frontend heraus. Über diesen MCP ist er weder lesbar noch schreibbar — ein solcher Tab ist NICHT ' +
    'leer, und dass hier keine Widgets stehen, ist kein Datenverlust. Zum Bearbeiten: im Admin anmelden, ' +
    'dann hat der Editor den Inhalt.';

/**
 * The refusal a locked view gets — for reading and for writing alike, naming what
 * to do instead. There is nothing here to measure and nothing to replace: a write
 * would land NEXT TO the protected content and be overwritten again on the next
 * unlock, which is worse than a plain no.
 */
function pinLockedRefusal(label, lock) {
    return (
        `${label} ist PIN-geschützt${lock && lock.scope === 'section' ? ' (der ganze Bereich)' : ''} — ` +
        'der Inhalt liegt server-seitig im Tresor, hier steht nur ein leerer Platzhalter. Darum ist hier ' +
        'weder etwas zu lesen noch zu schreiben: ein Schreibzugriff würde neben dem geschützten Inhalt ' +
        'landen und beim Entsperren wieder verschwinden. Im Editor bearbeiten (Admin-Anmeldung), oder die ' +
        'PIN vorher entfernen.'
    );
}
        ...
        for (const section of layout.sections || []) {
            for (const tab of section.tabs || []) {
                ...
                out.push({
                    ...
                    // Locked views keep their empty stub list — callers must check
                    // `pinLocked` before reading anything into it being empty.
...
/**
 * The one write path for the dashboard — and therefore the one place that decides
 * what happens to PIN-protected content.
 *
 * A protected section/tab is an empty stub in this state and its widgets live in
 * the vault (lib/mcp/protectedView). A model that was hydrated for a RELEASED view
 * carries those widgets, and they must not travel into `config.dashboard`, which
 * every connected socket client can read: flushHydrated writes them back into the
 * vault and hands over a copy in which the stub is empty again. Doing it here and
 * not in each tool is deliberate — a tool that forgot would either leak the
 * content or drop the change without a trace.
 */
async function writeDashboard(adapter, layouts, groupDefs) {
    ...
    const safe = flushHydrated(adapter, layouts);
    if (safe.refused.length && adapter.log) {
        // Widgets on a stub that nobody released. Nothing legitimate produces this,
        // so it is dropped rather than stored — and said out loud.
        adapter.log.warn(
            `aura MCP: widgets on a protected stub without a release were dropped (${safe.refused.join(', ')})`,
        );
    }
    env.state = Object.assign({}, env.state, { layouts: safe.layouts });
