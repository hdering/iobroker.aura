        ...
        app.get('/aura/proxy', (req, res) => {
            ...
            const proxyReq = lib.request(reqOptions, (proxyRes) => {
                ...
                if (contentType.includes('text/html')) {
                    // Must buffer HTML to inject <base> — remove content-length
                    ...
                    proxyRes.on('end', () => {
                        // Inject <base href="…"> so relative URLs resolve to the origin server
                        const baseTag  = `<base href="${targetUrl.toString()}">`;
                        const injected = body.replace(/<head(\s[^>]*)?>/i, m => `${m}${baseTag}`);
                        res.end(injected !== body ? injected : `${baseTag}${body}`);
                    });
                } else {
                    res.writeHead(proxyRes.statusCode || 200, resHeaders);
