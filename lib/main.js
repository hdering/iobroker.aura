...
class Aura extends utils.Adapter {
  ...
  async onReady() {
    ...
    // Update overview links when a custom base URL is configured.
    // Only write if the links have actually changed – otherwise ioBroker
    // would detect a config change and restart the adapter in a loop.
    if (this.config.customUrl) {
      const base = this.config.customUrl.replace(/\/+$/, '');
      const wantFrontend = `${base}/aura/`;
      const wantBackend  = `${base}/aura/#/admin`;
      try {
        const obj = await this.getForeignObjectAsync(`system.adapter.${this.namespace}`);
        const links = obj && obj.common && obj.common.localLinks || {};
        if (links['Aura Frontend'] !== wantFrontend || links['Aura Backend'] !== wantBackend) {
          await this.extendForeignObjectAsync(`system.adapter.${this.namespace}`, {
            common: {
              localLinks: {
                'Aura Frontend': wantFrontend,
                'Aura Backend':  wantBackend,
              },
            },
          });
          this.log.info(`localLinks updated to: ${base}`);
        }
      } catch (e) {
